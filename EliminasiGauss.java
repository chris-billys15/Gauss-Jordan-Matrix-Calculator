...
public class EliminasiGauss{
  ...
  public void solusiEG(double [][]A, double []B) throws Exception{
    int N = B.length;
    int M = A[0].length;
...
*/
        ...
        /*rekursi untuk OBE */
        for (int i = k + 1; i < N; i++){
          if (A[k][k] == 0) { /*mencegah elemen dibagi 0*/
            break;
          }
          else{
  ...
  public void cetakMatriksEselon(double[][] A, double []B){
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < M; j++){
        A[i][j] = A[i][j] / koefbagi; /*Membuat leading one*/
  ...
  public void cetakSolusi (double[] sol, double[]B) throws Exception {
    ...
    char variabel = 'r';
    FileWriter writer = new FileWriter("HasilGauss.txt");
